...
public class VehicleValueMapper implements DSValueMapper<Vehicle, VehicleResponse, VehicleRequest> {
    ...
    public static Vehicle responseToEntity(VehicleResponse dsVehicleResponse){
        return new Vehicle(dsVehicleResponse.getId(), dsVehicleResponse.getVin(), dsVehicleResponse.getLocCode(), dsVehicleResponse.getEndDate(), dsVehicleResponse.getStatus(), dsVehicleResponse.getAssemblyPlant());
    }

    public static VehicleResponse entityToResponse(Vehicle vehicle){
        return new VehicleResponse(vehicle.getId(), vehicle.getVin(), vehicle.getLocCode(), vehicle.getEndDate(), vehicle.getStatus(), vehicle.getAssemblyPlant());
    }
