...
public class LocationValueMapper implements DSValueMapper<Location, LocationResponse, LocationRequest> {
    ...
    public static Location responseToEntity(LocationResponse dsLocationResponse){
        return new Location(dsLocationResponse.getId(), dsLocationResponse.getLocType(), dsLocationResponse.getDescription(), dsLocationResponse.getEndDate());
    }

    public static LocationResponse entityToResponse(Location location){
        return new LocationResponse(location.getId(), location.getLocType(), location.getDescription(), location.getEndDate());
    }
}
